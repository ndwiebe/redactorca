...
const enabled = new Set(Object.keys(CATS));
const rejected = new Set(); // span keys the user manually turned off
let currentText = '';
let currentSpans = [];
function escapeHtml(s) {
    return s.replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]));
}
const spanKey = (s) => `${s.start}:${s.end}:${s.category}`;
...
function applyPreset(preset) {
    enabled.clear();
    Object.keys(CATS).forEach((cat) => {
        if (CATS[cat].presets.has(preset))
            enabled.add(cat);
    });
    buildToggles();
    render();
...
function renderSummary() {
    ...
    const chips = Object.entries(counts)
        .map(([c, n]) => `<span class="chip"><span class="dot" style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${CATS[c].color};margin-right:6px"></span>${CATS[c].label}: <b>${n}</b></span>`)
        .join('');
    $('#summary').innerHTML =
